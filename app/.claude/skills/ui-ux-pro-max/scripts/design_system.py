...
class DesignSystemGenerator:
    ...
    def _select_best_typography(self, results: list, mood_keywords: list) -> dict:
        """Select best typography match using mood keywords from reasoning rules."""
        if not results:
            return {}
        ...
        for result in results:
            score = 0
            mood_field = result.get("Mood/Style Keywords", "").lower()
            best_for = result.get("Best For", "").lower()
            notes = result.get("Notes", "").lower()
            name = result.get("Font Pairing Name", "").lower()

            for kw in mood_keywords:
                kw_lower = kw.lower().strip()
                # Highest: mood field match (this is the semantic connection)
                if kw_lower in mood_field:
                    score += 10
                # High: name match
                elif kw_lower in name:
                    score += 7
                # Medium: notes match (design reasoning)
                elif kw_lower in notes:
                    score += 3
                # Lower: best-for match
                elif kw_lower in best_for:
                    score += 1
            scored.append((score, result))
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        if typography_mood:
            mood_query = f"{query} {typography_mood}"
            mood_search = search(mood_query, "typography", SEARCH_CONFIG["typography"]["max_results"])
            mood_results = self._extract_results(mood_search)
            if mood_results:
                typography_results = mood_results
            # Extract mood keywords as priority list for best-match scoring
            mood_priorities = [m.strip() for m in typography_mood.replace("+", ",").split(",") if m.strip()]
            best_typography = self._select_best_typography(typography_results, mood_priorities)
        else:
