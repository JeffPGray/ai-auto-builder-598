...
If it's `cloudflare`, follow **§ Cloudflare Pages**. If it's `netlify`, follow **§ Netlify**. If it's `selfhost`, follow **§ Self-host (your own server)**. Anything else (including unset) means Vercel — follow **§ Vercel**. Do NOT run more than one. Do NOT switch hosts mid-deploy or fall back to another host on failure — the operator only has an account on the one they configured.
