...
import { getSettings, type BookingSettings } from "@/lib/booking/settings";
import {
  dayOfWeekForDateStr,
  isCutoffPassed,
  isValidDateStr,
...
/**
 * POST /api/booking — create a booking (auto-confirms).
 ...
 * (slot-capacity variant). Generic plumbing — do not edit per site.
 *
 * Two flows share the POST handler:
 *   1. Public web flow — honeypot + rate limit + origin check + strict
 *      validation; cutoff / service-day / window enforced. Sends owner
 *      notification + customer confirmation.
 *   2. Staff "phone booking" flow — requires a valid admin session cookie.
 *      Bypasses honeypot/rate-limit/origin and the cutoff (the RPC's
 *      admin_override); phone/email optional; customer confirmation opt-in;
 *      owner notification skipped (the owner is the one creating it).
 */

const BOOK_MAX = 5;
const BOOK_WINDOW_MS = 60 * 60 * 1000;

// Loose international phone shape — the pipeline is region-agnostic, so no
// country-specific regex here. Requires at least 5 digits somewhere (so
// "+---" can't pass); allows digits/spaces/+()-. and 5–25 chars total.
const PHONE_REGEX = /^(?=(?:[^0-9]*[0-9]){5})[+()0-9][()0-9\s.\-]{4,24}$/;

function makeSchemas(s: BookingSettings) {
  const common = {
    date: z
      .string()
      .regex(/^\d{4}-\d{2}-\d{2}$/, "Invalid date format")
      .refine(isValidDateStr, "Invalid date"),
    slotTime: z.string().refine((v) => s.slotTimes.includes(v), "Invalid time slot"),
    partySize: z.coerce
      .number()
      .int()
      .min(s.minParty, `Minimum party size is ${s.minParty}`)
      .max(s.maxParty, `Maximum party size is ${s.maxParty}`),
    name: z.string().trim().min(2, "Name must be at least 2 characters").max(100, "Name is too long"),
    specialRequests: z.string().trim().max(500, "Special requests must be under 500 characters").default(""),
  };
  ...
  return { publicSchema, adminSchema };
}
...
export async function POST(request: NextRequest) {
  ...
  const { publicSchema, adminSchema } = makeSchemas(s);
  ...
  if (isAdmin) {
    ...
    // Friendly pre-checks against the live settings. The RPC re-enforces all
    // of these transactionally — these exist only for better error copy.
    if (!s.serviceDays.includes(dayOfWeekForDateStr(data.date))) {
    ...
    if (isCutoffPassed(data.date, data.slotTime, s.cutoffHours, s.timezone)) {
      return NextResponse.json(
  ...
  for (let attempt = 0; attempt < MAX_REF_ATTEMPTS; attempt++) {
    ...
    // Translate the RPC's business errors into user-friendly responses.
    const msg = lastError.message ?? "";
    if (msg.includes("Slot full")) {
      // Strongest unmet-demand signal: the customer completed the form and
      // lost the race for the last capacity. Skip for admin (that's a known
      // double-book attempt, not unmet customer demand).
      if (!isAdmin) {
    ...
    if (msg.includes("Invalid party size")) {
      return NextResponse.json({ error: "That party size can't be booked online." }, { status: 400 });
    }
...
export async function GET(request: NextRequest) {
  ...
  let query = supabaseAdmin()
    .from("biz_bookings")
    .select(
      "id, booking_ref, booking_date, slot_time, party_size, tables_consumed, customer_name, customer_phone, customer_email, special_requests, visit_number, status, cancellation_reason, cancelled_by, created_at"
    );
  ...
  if (fromParam || toParam) {
    ...
    query = query.gte("booking_date", fromParam).lte("booking_date", toParam);
  } else if (dateParam) {
