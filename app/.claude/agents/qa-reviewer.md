...
**Call 1 — build (foreground), SKIPPED if `out/` is already fresh.** `/build`'s own Verify step
already ran `npx next build` moments before QA starts — rebuilding again here is a second full
Turbopack build + static export of the same source for zero new information, measured to cost
several real minutes on every single QA round. Only rebuild when `out/` could actually be stale
(round 2+ after `/qa-fix` touched source, or `out/` doesn't exist yet):
```bash
...
```bash
# Screenshots + the parallel gate battery + teardown. Extracted verbatim to a script
# 2026-08-19: 305 lines of pure orchestration needing zero model judgement, reloaded by the QA
# subagent every round. The script carries the load-bearing comments (port races, EXIT-trap
# rules); the model never has to read them.
bash ../../../scripts/qa-capture.sh {slug}
```
...
Review **every** screenshot in `clients/{slug}/site/qa-screenshots/` with the Read tool, AND the source code. That is the six home-page shots (`qa-top.webp`, `qa-mid.webp`, `qa-bottom.webp`, `qa-mobile-top.webp`, `qa-mobile-mid.webp`, `qa-mobile-bottom.webp`) **plus four per subpage** (`qa-services-top.webp`, `qa-services-bottom.webp`, `qa-services-mobile-top.webp`, `qa-services-mobile-bottom.webp`, and the same for `about` and `contact`). Run `ls clients/{slug}/site/qa-screenshots/` first and confirm you have opened every file it lists — the count is one of your Step 7 confirmations.

**Run the static, tree-wide checklist greps below in ONE Bash call now** (Fable consult, 2026-08-18 — several checklist items further down each separately tell you to run one of these; pre-running them in a batch means you reference the output instead of paying a separate tool-call turn per bullet). This does not replace the "Verify before reporting" rule above — if a result looks surprising, re-run that one command alone to double check before reporting it.
```bash
cd clients/{slug}/site
echo "--- fontFamily inline overrides (must be 0) ---"; grep -rc "fontFamily" src/app --include='*.tsx' | awk -F: '{s+=$2} END{print s+0}'
echo "--- malformed unicode escapes (must be empty) ---"; grep -rn '\\u20' src/app --include='*.tsx'
echo "--- footer legal links + files ---"; test -f out/privacy.html && test -f out/terms.html && echo "OK files exist" || echo "MISSING privacy/terms file"; grep -q 'href="/privacy"' out/index.html && grep -q 'href="/terms"' out/index.html && echo "OK footer links present" || echo "MISSING footer legal link"
echo "--- marquee animation duration ---"; grep -n 'animation: marquee' src/app/globals.css
echo "--- overflow-x: clip present ---"; grep -q 'overflow-x: clip' src/app/globals.css && echo "OK present" || echo "MISSING overflow-x: clip"
echo "--- scroll cue slop check ---"; grep -rn '>Scroll<\|scroll-cue' src/app _components 2>/dev/null
echo "--- h1 count per route (each must be 1) ---"
...
- [ ] **No thin stubs (hard FAIL).** Read the `WORDS_<route>=` values. Any marketing page under ~120 words is a stub — flag it critical and name the page. **The correct fix is to WRITE the missing content, not to delete the route** (changed 2026-08-19 on Jeff's direct instruction: every named service gets its own page, always, and content gets generated for it — multi-page is the standard shape, see build/SKILL.md § Per-service pages). Real bucket-2 trade-craft prose (how this specific job is actually done, what a customer should expect) is what fills a thin service page legitimately; fabricating business-specific facts to pad it is still forbidden and is a separate, worse failure.
...
- [ ] A serif/slab heading font is used per the build skill's font guidance (not Inter, Geist, Roboto, Arial, or system-ui). Acceptable heading fonts include Bitter, Fraunces, Literata, Zilla Slab, Vollkorn, Crimson Pro, Cormorant Garamond, Spectral, Eczar, Newsreader, Libre Caslon Display, Petrona, Brygada 1918, Instrument Serif, or any other distinctive serif/slab not on the banned list in `.claude/skills/build/SKILL.md`.
...
- [ ] Contact section is present. Per build/SKILL.md's § Contact form (2026-08-18 spec), it MUST POST to `/api/preview/${slug}/contact` (same-origin fetch) — `grep -n "mailto:" src/app/_components/ContactForm.tsx` should return nothing, and `grep -n "/api/preview/" src/app/_components/ContactForm.tsx` should. A form that just shows an optimistic success message with no real POST, or one wired as `mailto:`, is a critical FAIL regardless of whether an email is known — the endpoint's own orphan-queue path already covers the no-email case, so there is no legitimate "no email, so stub it" exception anymore.
...
- [ ] **Body typography, measured with getComputedStyle (CSS grep cannot see it).** Read the
      `TYPE_<route>=<font-size>/<font-weight>` values already captured in Step 3 (folded into the
      same loop that takes screenshots — no separate browser call needed here).
...
- [ ] **RENDERED depth, counted in the browser (not from CSS).** Read the `DEPTH_<route>=` values
      already captured in Step 3 (one per route, folded into the same loop that takes screenshots —
      no separate browser call needed here). **CSS declarations overstate reality** — measured on a
      live build, 9 gradient declarations painted only 2 elements, while grain correctly rendered 7
      at 0.12/0.16. Expect >=3 painting gradients and grain on every flat section. Also count
      DISTINCT section background colours from the screenshots themselves: a long page wants 5+, and
      the surface ladder makes that achievable without a second hue.
...
- [ ] **Mobile horizontal overflow.** Read the JSON output of the overflow-check eval from Step 3. The list MUST be empty. Any non-empty result means at least one element extends past the mobile viewport — this manifests as body text being clipped mid-word and is a critical issue. Look at `qa-mobile-top.webp`, `qa-mobile-mid.webp`, `qa-mobile-bottom.webp` and confirm: every paragraph wraps cleanly within the visible width, no headline overflows the right edge, no horizontal scrollbar at the bottom of the screenshot. If anything looks cut off, flag as critical and name the offending element from the JSON. Belt-and-braces fix the build skill enforces: `html, body { overflow-x: clip; }` in globals.css. Confirm that rule is present — if absent, flag.
...
- [ ] **Composition judgement via `impeccable` (hard FAIL) — the gate class this pipeline has never had.** Every other gate here measures PRESENCE or VALIDITY, and the operator's own recorded lesson is that correctness gates structurally cannot see flatness. After the screenshots are captured, invoke:
  `Skill(skill="impeccable", args="critique — judge these screenshots of a {TRADE} marketing site against award-tier work for this trade. Answer three questions only: (1) does the execution match the stated DESIGN_IDEA in status.md, or is the idea absent from the pixels? (2) would this font pairing and palette be plausible on an award-tier site FOR THIS TRADE, or does either read as borrowed from another category? (3) is any section a repeated uniform list with no hierarchy?")`
  **Any contradiction it names is a CRITICAL FAIL.** This exists because a build recorded a coherent brief, passed all 13 deterministic gates, and the operator called the result AI slop on sight — the machine had no way to notice that "engineered precision instrument panel" had shipped as a Vogue didone over grey-on-black menu rows. Do NOT soften its verdict because the numeric gates are green; green numbers are precisely what failed. Report its findings verbatim.
...
Read the identical 6-check contract from `build/SKILL.md` § "HARD-BLOCKER CONTRACT" — it is the
...
- **Screenshots taken**: confirm that `npx next build` succeeded, the local http.server served `out/`, and npx playwright-cli wrote qa-top.webp, qa-mid.webp, qa-bottom.webp AND qa-mobile-top.webp, qa-mobile-mid.webp, qa-mobile-bottom.webp
