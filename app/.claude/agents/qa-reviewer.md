...
( cd ../../.. && node scripts/font-uniqueness-check.mjs {slug} ) || true
...
( cd ../../.. && node scripts/hyperui-transplant-check.mjs {slug} ) || true
...
( cd ../../.. && node scripts/copy-fingerprint-check.mjs {slug} ) || true
...
- [ ] **HyperUI transplant fidelity (hard FAIL on this branch, N/A elsewhere) — read the `HYPERUI_TRANSPLANT_CHECK=` line printed by `hyperui-transplant-check.mjs` in Step 3.** `SKIP` means the vendored reference set isn't present, or `clients/{slug}/site/out` doesn't exist yet (pre-build) — not a finding. `FAIL` on that branch is CRITICAL and hard, always report it verbatim: it means a cited path doesn't exist in the vendored set, or a `data-hyperui` stamp shipped in the built HTML that status.md never cited — both are unambiguous bookkeeping lies (proof exists, not a scoring judgement), fix by correcting whichever side is wrong (add the missing citation, or fix the invalid path) so they agree. `WARN` (an `UNSTAMPED` citation — cited in status.md but no matching `data-hyperui` anywhere in the build — or a `DECORATIVE` structural-similarity score) is NOT a hard FAIL yet: this is a brand-new stamping convention with no calibration data from a real build, so report it verbatim but don't fail the round on it alone. **Do hand-inspect any `DECORATIVE` verdict against the actual rendered section anyway** — the script's own header says this signal needs human eyes before it earns a hard-FAIL promotion, and an operator override on a bad score is exactly the calibration data it's waiting on. Fix for `UNSTAMPED` is adding the `data-hyperui="<exact cited path>"` attribute to the shipped element's root per `build/SKILL.md`'s HyperUI section; fix for `DECORATIVE` is either genuinely rebuilding that section from the cited file's structure, or citing a different file that actually matches what shipped.
